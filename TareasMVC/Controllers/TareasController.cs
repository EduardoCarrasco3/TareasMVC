...
namespace TareasMVC.Controllers
{
    ...
    public class TareasController : ControllerBase
    {
        ...
        public async Task<ActionResult<Tarea>> Post([FromBody] string titulo)
        {
            ...
            var existenTareas = await context.Tareas
                .AnyAsync(t => t.UsuarioCreacionId == usuarioId);
        ...
        public async Task<IActionResult> Ordenar([FromBody] int[] ids)
        {
            ...
            var idsTareasNoPertenecenUsuario = ids.Except(tareasId).ToList();
            if (idsTareasNoPertenecenUsuario.Any())
            {
                return Forbid();
            }
            var tareaDiccionario = tareas.ToDictionary(x => x.Id);

            for (int i = 0; i < ids.Length; i++)
            {
                var id = ids[i];
                var tarea = tareaDiccionario[id];
                tarea.Orden = i + 1;
            }
            await context.SaveChangesAsync();
            return Ok();
